...
<div class="container py-4">
    ...
    {{-- Form action points to the update route with the shipment's ID --}}
    <form action="{{ route('pengiriman.update', $pengiriman->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            {{-- Hidden inputs for actual values to be submitted --}}
            <input type="hidden" name="berat_per_unit" id="berat_per_unit"
                   value="{{ old('berat_per_unit', $pengiriman->berat_per_unit) }}">
            <input type="hidden" name="total_berat" id="total_berat"
                   value="{{ old('total_berat', $pengiriman->total_berat) }}">
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="kendaraan_id" id="kendaraan_id" class="form-select" required>
                ...
                @foreach($kendaraans as $kendaraan)
                    <option
                        ...
                        {{ old('kendaraan_id', $pengiriman->kendaraan_id) == $kendaraan->id ? 'selected' : '' }}>
                        {{ $kendaraan->nama }}
                    </option>
                @endforeach
            </select>
            ...
            <input type="hidden" name="plat_nomor" id="plat_nomor"
                   value="{{ old('plat_nomor', $pengiriman->plat_nomor) }}">
            <input type="hidden" name="ukuran_kendaraan" id="ukuran_kendaraan"
                   value="{{ old('ukuran_kendaraan', $pengiriman->ukuran_kendaraan) }}">
            <div class="mt-2">
        ...
        <div class="mb-3">
            ...
            <input type="datetime-local" name="jadwal_kirim" id="jadwal_kirim" class="form-control" required
                   value="{{ old('jadwal_kirim', \Carbon\Carbon::parse($pengiriman->jadwal_kirim)->format('Y-m-d\TH:i')) }}">
        </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        let pusatMarker = null;         // Stores the Leaflet marker for the central location
        ...
        // Initialize destination marker with existing data or default to center
        ...
        destinationMarker.on('dragend', function (e) {
            ...
            reverseGeocode(pos.lat, pos.lng); // Get address from new coordinates
            generateRoute();
        });
        ...
        // or on initial page load when editing.
        produkSelect.dispatchEvent(new Event('change'));
        kendaraanSelect.dispatchEvent(new Event('change'));

        // Handle initial map and route setup for existing shipment data
        // This must be done AFTER the driverSelect has dispatched its change event
        // to ensure latitudeAwalInput and longitudeAwalInput are populated.
        const currentDriverLat = parseFloat(latitudeAwalInput.value);
        const currentDriverLng = parseFloat(longitudeAwalInput.value);
        const currentTujuanLat = parseFloat(latitudeTujuanInput.value);
        const currentTujuanLng = parseFloat(longitudeTujuanInput.value);

        if (!isNaN(currentTujuanLat) && !isNaN(currentTujuanLng)) {
            // Update destination marker position
            destinationMarker.setLatLng([currentTujuanLat, currentTujuanLng]);
            // Reverse geocode to update the 'tujuan' text field
            reverseGeocode(currentTujuanLat, currentTujuanLng);

            // If driver location is also available, generate the route
            if (!isNaN(currentDriverLat) && !isNaN(currentDriverLng)) {
                // Dispatch driverSelect change to ensure driverMarker is set and then generate route
                // This ensures the driver's status and coordinates are correctly set before routing.
                // We'll simulate a change event on the selected driver option.
                const selectedDriverOption = driverSelect.querySelector(`option[value="${driverSelect.value}"]`);
                if (selectedDriverOption) {
                    const event = new Event('change');
                    driverSelect.dispatchEvent(event);
                } else {
                    // Fallback if no driver selected or selected driver not found, but coords exist
                    generateRoute();
                }
            } else {
                // If destination is set but driver is not, pan map to destination
                map.setView([currentTujuanLat, currentTujuanLng], 13);
            }
        } else if (!isNaN(currentDriverLat) && !isNaN(currentDriverLng)) {
            // If only driver is selected initially, pan map to driver's location and set driver marker
            map.setView([currentDriverLat, currentDriverLng], 13);
            if (driverMarker) map.removeLayer(driverMarker);
            driverMarker = L.marker([currentDriverLat, currentDriverLng]).addTo(map).bindPopup("Lokasi Driver").openPopup();
        }
    });
</script>
@endpush
